...
public class Ex02_Arrays {
	...
	public static void main(String[] args) {		
		 ...
		 * rrays클래스를 이용하면 배열의 기능을 더욱 쉽게 사용할 수 있다.
		 * 
		 * Scanner, Random, 클래스의 경우 객체를 먼저 생성을 하고 메서드를 사용함
		 * Arrays.함수명()으로 작성하여 기능을 호출한다.
		 * 
		 * 1. 배열의 출력
		 * toString()
		 * 반복문의 도움 없이 배열을 출력할 수 있도록 도와준다.
		 * 배열에 정의 된 값을 문자열 형태로 변환하여 출력해준다.
		 */
		 ...
		 * 하지만 기본자료형 배열로는 불가능 하다.
		 * 기본자료형의 클래스 타입인 Wrapper클래스를 이용한다.
		 * 
		 * int -> Integer
		 */
		 ...
		 * 
		 * 얕은복사(Shallow Copy) : 복사된 배열이나 원본 배열이 변경될때 서로간의 값이 함께 변경된다.
		 * 
		 * 깊은복사(Deep Copy) : 복사된 배열인 ㅏ원본 배열이 변경될 때 서로 간의 값은 바뀌지 않는다.
		 */
		
		int[] arr01 = {1,2,3};
		//arr01 배열 {1,2,3}의 주소값을 가지고 있다.
		...
		System.out.println("배열 arr02의 1번 배열 값을 10으로 변경");
		arr02[1] = 10; 
		System.out.println("arr01 배열 : " + Arrays.toString(arr01));//arr02를 바꿨지만 얕은 복사이기 때문에 arr01도 같이 바뀜
		System.out.println("arr02 배열 : " + Arrays.toString(arr02));
